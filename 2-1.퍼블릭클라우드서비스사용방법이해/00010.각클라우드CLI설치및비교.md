...
AWS CLI v2 설치 도중 멈춤 현상(Freezing)은 주로 윈도우(Windows) 환경에서 **권한 승인 창이 가려져 있거나**, 맥(Mac)에서 **패키지 업데이트 대기** 중에 자주 발생합니다.

현재 사용 중인 운영체제(OS)에 맞춰 아래 단계를 순서대로 조치해 보세요.

---

### 1. Windows 사용자 해결 방법

대부분의 경우 **"사용자 계정 컨트롤(UAC)"** 팝업창이 다른 창 뒤에 숨어서 "예(Yes)"를 누르기를 기다리고 있는 상황입니다.

#### **단계 1: 숨겨진 창 확인**

* `Alt` + `Tab` 키를 눌러 화면에 보이지 않는 **권한 승인 요청(방패 모양 아이콘)** 창이 있는지 확인합니다. 있다면 선택하여 **[예]**를 누르세요.

#### **단계 2: 강제 종료 후 재시도**

숨겨진 창이 없다면 설치 프로그램이 꼬인 상태입니다.

1. `Ctrl` + `Shift` + `Esc`를 눌러 **작업 관리자**를 엽니다.
2. `Windows Installer` 또는 `msiexec.exe` 프로세스를 찾아 **[작업 끝내기]**를 합니다.
3. 다운로드받은 `.msi` 파일을 다시 실행하지 말고, **컴퓨터를 재부팅** 한 번 한 뒤 재설치합니다.

#### **단계 3: (추천) 명령 프롬프트로 설치**

GUI(마우스 클릭) 설치가 계속 멈춘다면, 터미널 명령어로 설치하는 것이 가장 확실합니다.

1. **PowerShell**을 **관리자 권한**으로 실행합니다. (시작 메뉴 > PowerShell 우클릭 > 관리자 권한으로 실행)
2. 아래 명령어를 복사해서 붙여넣고 엔터를 칩니다. (자동으로 다운로드 및 설치가 진행됩니다)
```powershell
...
* `/qn` 옵션은 화면 없이 백그라운드에서 조용히 설치하라는 뜻입니다.
* 명령어 입력 후 약 1~2분 정도 기다린 뒤 프롬프트가 다시 뜨면 설치가 끝난 것입니다.
...
#### **Homebrew로 설치 중 멈춘 경우**

터미널에 `brew install awscli`를 입력했는데 멈췄다면, 대부분 Homebrew 자체 업데이트(`Updating Homebrew...`)가 오래 걸리는 것입니다.

1. `Ctrl` + `C`를 눌러 현재 작업을 취소합니다.
2. 다시 명령어를 입력하되, 업데이트를 건너뛰는 옵션을 사용합니다.
```bash
...
* **보안 설정 확인:** `시스템 설정` > `개인정보 보호 및 보안`에서 설치 프로그램 실행이 차단되었는지 확인하고 **[확인 없이 열기]**를 클릭해야 할 수 있습니다.

---

### 3. 설치 성공 확인

조치 후 터미널(CMD 또는 PowerShell)을 **새로 열어서** 아래 명령어를 입력했을 때 버전 정보가 나오면 성공입니다.
...
* **정상 출력 예시:** `aws-cli/2.15.xx Python/3.11.xx Windows/10 ...`
