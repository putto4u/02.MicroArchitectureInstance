...
## 하이브리드 멀티 클라우드 표준화 정의서 (시나리오 예시)

본 문서는 **온프레미스(IDC)**와 **3대 클라우드(AWS, Azure, GCP)**를 모두 활용하는 하이브리드 환경을 가정합니다. 두 개의 프로젝트(**쇼핑몰, 데이터 분석**)를 수행하기 위한 웹 서버, DB 서버, 스토리지의 통합 운영 표준을 정의합니다.

---

### 1. 시나리오 및 프로젝트 정의

* **인프라 구성:** 4개 환경(IDC, AWS, Azure, GCP)에 각각 Web 1대, DB 1대, Storage 1대 배치.
* **프로젝트 A (코드명: `shop`):** 대고객 쇼핑몰 서비스 (안정성 중요).
* **프로젝트 B (코드명: `data`):** 로그 수집 및 데이터 분석 플랫폼 (성능 중요).
* **목표:** IaC(Terraform, Ansible) 도구 하나로 모든 환경을 제어하기 위한 완벽한 **Naming 및 환경 통일**.

---

### 2. 글로벌 거버넌스 (Global Governance)

모든 환경에 공통으로 적용되는 불변의 법칙입니다.

| 분류 | 항목 | 표준 정의 값 | 비고 |
| --- | --- | --- | --- |
| **OS 표준** | 운영체제 | **Ubuntu Server 24.04 LTS** | 모든 클라우드/IDC 통일 (커널 호환성 확보) |
| **계정 관리** | 관리자 ID | **`devops-admin`** | `root` 사용 금지, `ec2-user` 등 CSP 기본 계정 미사용 |
| **인증 방식** | SSH Key | **`hybrid-master-key.pem`** | 단일 개인키로 모든 서버 접속 (Vault 관리 필수) |
| **IaC 도구** | Terraform | **v1.9.x** (Provider 버전 고정) | 버전 파편화 방지 |
| **리소스 명명** | Naming Rule | `[위치]-[환경]-[프로젝트]-[역할]-[순번]` | 예: `aws-prd-shop-web-001` |

---

### 3. 네트워크 IP 할당 전략 (IPAM)

VPN 또는 전용선(Direct Connect) 연결 시 충돌을 방지하기 위해 CIDR 대역을 철저히 분리합니다.

| 위치 (Location) | 위치 코드 | 할당 CIDR (VPC/Network) | 서브넷 전략 (Subnet) |
| --- | --- | --- | --- |
| **On-Premise** | `idc` | **10.0.0.0/16** | 10.0.1.0 (Web), 10.0.2.0 (DB) |
| **AWS (Seoul)** | `aws` | **10.10.0.0/16** | 10.10.1.0 (Web), 10.10.2.0 (DB) |
| **Azure (Korea)** | `azr` | **10.20.0.0/16** | 10.20.1.0 (Web), 10.20.2.0 (DB) |
| **GCP (Seoul)** | `gcp` | **10.30.0.0/16** | 10.30.1.0 (Web), 10.30.2.0 (DB) |

---

### 4. 리소스 표준화 및 배포 상세 명세서

각 환경별로 프로젝트 A(`shop`)와 프로젝트 B(`data`)를 분산 배치하여 운영하는 시나리오의 상세 스펙입니다.

#### [범례]

* **환경:** `prd` (운영 환경)
* **역할:** `web` (Nginx), `db` (PostgreSQL/MySQL), `st` (Object Storage/NAS)

| 환경 | 리소스 구분 | 프로젝트 | 리소스 이름 (Hostname/ID) | 사설 IP (예시) | OS / 엔진 버전 | 스펙 / 유형 |
| --- | --- | --- | --- | --- | --- | --- |
| **1. IDC**<br>

<br>(메인 센터) | **Web Server** | `shop` | **`idc-prd-shop-web-001`** | 10.0.1.10 | Ubuntu 24.04 | Physical / VM |
|  | **DB Server** | `shop` | **`idc-prd-shop-db-001`** | 10.0.2.10 | MariaDB 10.11 | Physical (High-Mem) |
|  | **Storage** | `shop` | **`idc-prd-shop-st-001`** | 10.0.5.10 | TrueNAS (NFS) | NAS Appliance |
| **2. AWS**<br>

<br>(DR/트래픽 분산) | **Web Server** | `shop` | **`aws-prd-shop-web-001`** | 10.10.1.10 | Ubuntu 24.04 | EC2 (t3.medium) |
|  | **DB Server** | `shop` | **`aws-prd-shop-db-001`** | 10.10.2.10 | MariaDB 10.11 | RDS for MariaDB |
|  | **Storage** | `shop` | **`aws-prd-shop-st-001`** | (Endpoint) | S3 Standard | S3 Bucket |
| **3. Azure**<br>

<br>(데이터 분석) | **Web Server** | `data` | **`azr-prd-data-web-001`** | 10.20.1.10 | Ubuntu 24.04 | VM (Standard_B2s) |
|  | **DB Server** | `data` | **`azr-prd-data-db-001`** | 10.20.2.10 | PostgreSQL 16 | Azure Database |
|  | **Storage** | `data` | **`azr-prd-data-st-001`** | (Endpoint) | Blob Storage | Storage Account |
| **4. GCP**<br>

<br>(AI/ML 학습) | **Web Server** | `data` | **`gcp-prd-data-web-001`** | 10.30.1.10 | Ubuntu 24.04 | Compute Engine (e2) |
|  | **DB Server** | `data` | **`gcp-prd-data-db-001`** | 10.30.2.10 | PostgreSQL 16 | Cloud SQL |
|  | **Storage** | `data` | **`gcp-prd-data-st-001`** | (Endpoint) | GCS Standard | Cloud Storage |

---

### 5. Terraform Provider 및 버전 통일 전략

IaC 코드 작성 시 각 클라우드 제공자(Provider)의 버전을 명시하여, 협업 시 버전 차이로 인한 문법 오류를 방지합니다.

```hcl
# versions.tf (통합 관리 파일 예시)

terraform {
  required_version = ">= 1.9.0"  # Terraform 코어 버전 고정

  required_providers {
    # AWS Provider (v5.x 고정)
    aws = {
      source  = "hashicorp/aws"
      version = "~> 5.30"
    }
    # Azure Provider (v3.x 고정)
    azurerm = {
      source  = "hashicorp/azurerm"
      version = "~> 3.80"
    }
    # GCP Provider (v5.x 고정)
    google = {
